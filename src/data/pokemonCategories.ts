...
export const legendaryPokemonByGeneration: GenerationGroup[] = [
...
export const mythicalPokemonByGeneration: GenerationGroup[] = [
...
export const starterSpeciesIds = new Set<number>([
  1, 4, 7,
  152, 155, 158,
  252, 255, 258,
  387, 390, 393,
...
export const ultraBeastSpeciesIds = new Set<number>([793, 794, 795, 796, 797, 798, 799, 803, 804, 805, 806])

export const paradoxSpeciesIds = new Set<number>([
...
export function isRegionalFormName(name = ''): boolean {
  return regionalFormTokens.some((token) => name.includes(token))
}

export interface CategoryFlagsInput {
  apiName?: string
  isBaby?: boolean
  isLegendary?: boolean
  isMega?: boolean
...
export function getPokemonCategoryFlags({
  apiName = '',
  isBaby = false,
  isLegendary = false,
  isMega = false,
  ...
  isPrimal = false,
  speciesId,
}: CategoryFlagsInput): CategoryFlags {
  return {
