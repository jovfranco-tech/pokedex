...
const makeEntry = (overrides: Record<string, unknown> = {}) => ({
  ...
  capturedAt: undefined as string | undefined,
  ...overrides,
})
...
describe('useAchievements', () => {
  ...
  it('unlocks seen_10 only when 10+ entries have seenAt', () => {
    ...
    expect(r9.current.find((a) => a.id === 'seen_10')?.unlocked).toBe(false)
    expect(r10.current.find((a) => a.id === 'seen_10')?.unlocked).toBe(true)
  })
